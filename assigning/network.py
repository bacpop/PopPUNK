...
def updateClustering(dbPrefix,existingHits):
    ...
    for query in existingHits:
        ...
        if len(existingHits[query].keys()) >= 1:
            printAssignation(query,existingHits[query],newFile,superGroups,newClusterTranslation)
...
def findQueryLinksToNetwork(rlist,qlist,kmers,assignments,weights,means,covariances,outPrefix):
    ...
    if len(unassigned) > 0:
        ...
        print("starting here: "+tmpFileName+","+tmpDirString)
        constructDatabase(tmpFileName,kmers,sketchSize,tmpDirString)
        qlist1,qlist2,distMat = queryDatabase(tmpFileName,kmers,tmpDirString)
    ...
    for k in klist:
        ...
        for line in dbInfo.split("\n"):
            if (line[0] != "#"):
                ...
                if oldSketch == 0:
                    oldSketch = sketchValues[0]
                else:
                    oldSketch = sketch
                sketch = sketchValues[0]
                if (sketch != oldSketch):
