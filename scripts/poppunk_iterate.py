...
def get_options():
    ...
    parser = argparse.ArgumentParser(description='Cluster QC and analysis from multi-boundary method',
                                     prog='iterate')

    # input options
    parser.add_argument('--db', required=True, help='Output directory with results of --multi-boundary')
    parser.add_argument('--output', default=None, help='Prefix for output files [default = <--db>/<--db>_iterate')
    parser.add_argument('--cpus', default=1, type=int, help='Number of CPUs to use [default = 1]')
...
if __name__ == "__main__":
    ...
    for (all_clusters, no_singletons, refine_idx) in cluster_it:
        for new_cluster in no_singletons.values():
            ...
            for old_cluster in iterated_clusters.values():
                if new_cluster == old_cluster or not (new_cluster.issubset(old_cluster) or old_cluster.issubset(new_cluster) or len(new_cluster.intersection(old_cluster)) == 0):
                    valid = False
                    break
            if valid:
                cluster_idx += 1
                iterated_clusters[cluster_idx] = new_cluster
    sorted_clusters = sorted(iterated_clusters, key=lambda k: len(iterated_clusters[k]), reverse=True)

    # Calculate core distances
    # Set up sketchlib args
    ref_db = args.db + "/" + os.path.basename(args.db) + ".h5"
    ref_db_handle = h5py.File(ref_db, 'r')
    first_sample_name = list(ref_db['sketches'].keys())[0]
    kmers = ref_db['sketches/' + first_sample_name].attrs['kmers']
    ...
    for cluster in sorted_clusters:
        ...
        distMat = pp_sketchlib.queryDatabase(ref_db, ref_db, rNames, rNames, kmers,
                                             random_correct, jaccard, args.cpus,
                                             use_gpu, deviceid)
    ...
    for cluster in sorted_clusters[1:]:
        ...
        sub_cluster = is_nested(iterated_clusters, iterated_clusters[cluster], node_list.keys())
        if sub_cluster:
    ...
    for leaf in leaves:
        ...
        for sample in iterated_clusters[cluster]:
            leaf.add_node(Node(label=sample))
    ...
    with open(args.output + '.clusters.csv', 'w') as f:
        ...
        for cluster in sorted_clusters:
            f.write(str(cluster) + ',' + str(pi_values[cluster]) + ',' + \
